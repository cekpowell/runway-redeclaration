...
public class TopDownMap extends BorderPane {
    ...
    public static TopDownMap instance;

    private final Rectangle runwayRep;
    private final Group root;
    private final HBox buttonTray;
    ...
    void emplaceObstacle() {
        double runwayPosX = runwayRep.getX();
        double runwayPosY = runwayRep.getY();
        double runwayLen = runwayRep.getWidth();
        double runwayHeight = runwayRep.getHeight();

        double actualRunwayLen = runway.getParameters().getTora();

        double calc = runwayLen * revisedRunway.getObstaclePosition().getDistanceFromThreshold() / actualRunwayLen;
        ObstacleSpot obs = new ObstacleSpot(runwayPosX + calc, runwayPosY + runwayHeight / 2, runwayHeight / 3, Color.RED);
    ...
    void gradeArea() {
        ...
        if(revisedRunway.getFlightMethod() == FlightMethod.LANDING_OVER) {
            ...
            double alsLen = runwayLen * (revisedRunway.getAlstocs() / runway.getParameters().getTora());

            double resaLen = runwayLen * (revisedRunway.getResaDistance() / runway.getParameters().getTora());

            double longerLen = Math.max(resaLen, alsLen);
            double alsPos = obsPos - longerLen;
            double resaPos = obsPos - longerLen;
            ...
            double lowerOffset = runwayRep.getY() + runwayRep.getHeight() + 10;

            arrow = new TopDownArrow(alsPos, alsPos + alsLen, lowerOffset, lowerOffset, lowerOffset + 10);
            label = new Text();
            label.setText(String.format("ALS %dm", (int) revisedRunway.getAlstocs()));
            label.setX(alsPos);
            label.setY(lowerOffset + 15);
            ...
            label.setX(runwayRep.getX() + todaLen / 4);
            label.setY(yOffset - 10);

            root.getChildren().addAll(arrow, label);
            runwayLabels[5] = label;
        }
            ...
            */
            ...
            double longerLen = Math.max(resaLen, alsLen);
            double longerPos = obsPos + longerLen;

            double alsPos = longerPos - alsLen;
            double resaPos = longerPos - resaLen;
    ...
    public TopDownMap(LogicalRunway original, RevisedLogicalRunway revisedLogicalRunway, double initWidth, double initHeight) {
        ...
        Group legendGroup = TopDownViewDetails.getLegend(TopDownViewDetails.getGradedArea(rectangle));
        ...
        buttonTray = new HBox();
        Button setToTop = new Button("Rotate to Heading");
        setToTop.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent actionEvent) {
                if(setToTop.getText().equals("Normalize Rotation")) {
                    double heading = normaliseHeading(9);
                    root.setRotate(heading);
                    rot = heading;
                    reorientLabels();
                    provisionalZoomForRotation();
                    setToTop.setText("Rotate to Heading");
                }
                else {
                    double heading = normaliseHeading(runway.getHeading());
                    root.setRotate(heading);
                    rot = heading;
                    reorientLabels();
                    provisionalZoomForRotation();
                    setToTop.setText("Normalize Rotation");
                }
        ...
        Button legend = new Button("Show Legend");
        ...
        Button[] allButtons = {setToTop, lockRotation, legend};

        for(Button button : allButtons) {
            button.setMaxWidth(Double.MAX_VALUE);
        }

        buttonTray.getChildren().addAll(setToTop, lockRotation, legend);
    ...
    public WritableImage focusedSnapshot() {
        ...
        return root.snapshot(param, null);
    }
}  